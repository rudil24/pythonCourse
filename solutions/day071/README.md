...
7. sign up (or use your existing account) for one of the Python-ready providers:
   ...
   3. [Cyclic](https://www.cyclic.sh/pricing/) **Cyclic is shutting down, no new apps as of 2024**
...
8. Create a new "web service" with your provider (or whatever the provider's language for that. Before hitting the final "create web service" or "deploy"):
   ...
   5. link the PostGres database to your web service
       ...
       * paste the provider's db address it into your environment variable, but if it starts with postgres:// change it to **postgresql://**  (required for SQLAlchemy)
       * *WAIT! didn't we build our app with an SQLite db?* **That's OK:** The reason we can seamlessly switch from SQLite to Postgres is because we are using the psycopg package in combination with SQLAlchemy.
