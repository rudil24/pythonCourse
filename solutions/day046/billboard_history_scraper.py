...
class BillboardHistoryScraper:
    ...
    def _fetch_gid_map(self):
        print("Fetching sheet directory (Version: CSV Strategy + Rank Sort)...")
        try:
    ...
    def get_chart(self, target_date_str):
        ...
        for i, row in enumerate(rows[:10]):
            ...
            # We explicitly look for '#' (Rank) and 'Song Title'
            if "song title" in row_lower:
                for idx, col_name in enumerate(row):
                    clean_name = col_name.strip()
                    if clean_name:
                        header_map[idx] = clean_name
                start_row = i + 1
                break
        ...
        for idx, name in header_map.items():
            if "Date" in name:
                date_col_idx = idx
            # The column is usually named "#" or "No."
            elif "#" in name or "No." in name:
                rank_col_idx = idx
        ...
        for r_date, row in valid_rows:
            if r_date == closest_date:
                ...
                # Add a cleaner 'rank' integer for sorting
                # Default to 999 if missing or weird
                try:
                    if rank_col_idx != -1 and rank_col_idx < len(row):
                        entry["rank_int"] = int(row[rank_col_idx])
                    else:
                        entry["rank_int"] = 999
                except:
                    entry["rank_int"] = 999
        ...
        # --- SORT BY RANK ---
        final_chart.sort(key=lambda x: x["rank_int"])
